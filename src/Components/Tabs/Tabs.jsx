...
export default function Tabs() {
  ...
  const eventosFuturos = eventosState
    .filter((evento) => new Date(evento.dataTermino) > new Date())
    .sort((a, b) => b.count - a.count); // Ordena por count decrescente

  const eventosPassados = eventosState
    .filter((evento) => new Date(evento.dataTermino) < new Date())
    .sort((a, b) => b.count - a.count); // Ordena por count decrescente
          ...
          {eventosFuturos.map((evento) => (
            <div className={s.ranking} key={evento.id} id={evento.id}>
              ...
              <div>
                ...
                <br />
                <span>
                  Data: {formatarData(evento.dataInicio)} a{" "}
                  {formatarData(evento.dataTermino)}
                </span>
                <br />
